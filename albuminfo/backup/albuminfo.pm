...
my $albuminfowidget =
{	class			=> __PACKAGE__,
	tabicon			=> 'plugin-artistinfo-allmusic',
...
# 	my $url;
# 	my $waiting
# 	foreach my $id (@$::Library) {
# 		$aid = Songs::Get_gid($ID,'album');
# 		$album = lc(::url_escapeall(Songs::Gid_to_Get("album",$aID)));
# 		$url = "http://allmusic.com/search/album/$album";
# 		warn "Albuminfo: fetching search results from url $url\n" if $::debug;
# 		::IdleDo('8_albuminfo_download_all',1000, 
# 				 sub {$self->{waiting} = Simple_http::get_with_cb(cb=>
# 					sub {my $result = parse_amg_search_results($html, $type);
...
sub new {
	...
	$scrwin ->signal_connect(key_press_event => sub {$self->entry_selected_cb() if $_[1]->keyval == $Gtk2::Gdk::Keysyms{Return};}); # Pressing Enter in results list
...
#####################################################################
sub manual_search {
	my $self = shift; # The context pane object is needed when we get to load_review
	$self->{infoview}->hide();
...
sub new_search {
	...
	$self->{resultsbox}->pack_start(Gtk2::Label->new('Searching...'),0,0,20);
...
sub entry_selected_cb {
	...
	$self->cancel();
	$self->{waiting} = Simple_http::get_with_cb(cb=>sub {$self->load_review(@_)}, url=>$self->{url}, cache=>1);
}
...
sub load_file {
	...
	warn "Albuminfo: review = $self->{fields}{review}\n" if $::debug;
	$self->print_review();
...
sub save_review {
	...
	my $text = "";
	for my $key (sort {lc $a cmp lc $b} keys %{$self->{fields}}) {
		if ($key =~ m/genre|mood|Style|Theme/) {
...
sub save_fields {
	my ($self) = @_;
	my $IDs = Songs::MakeFilterFromGID('album', Songs::Get_gid(::GetSelID($self),'album'))->filter(); # Songs on this album
	my @updated_fields;
	for my $key (keys %{$self->{fields}}) {
		if ($key =~ m/mood|Style|Theme/ && $::Options{OPT.$key} && $self->{fields}->{$key}) {
			if ( $::Options{OPT.'ReplaceFields'} ) {
				push(@updated_fields, $key, $self->{fields}->{$key});
			} else {
				push(@updated_fields, '+'.$key, $self->{fields}->{$key});
			}
		}		
